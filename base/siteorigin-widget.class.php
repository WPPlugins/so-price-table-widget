...
abstract class SiteOrigin_Widget extends WP_Widget {
	...
	public function get_instance_css( $instance ){
		...
		$less = file_get_contents( $this->base_folder.'styles/'.$style_name . '.less' );
	...
	public function sanitize( $instance, $fields ) {
		foreach($fields as $name => $field) {
			...
			elseif($field['type'] == 'select') {
				// TODO Make sure that the value is in the options
			}
	...
	function render_field( $name, $field, $value, $repeater = array() ){
		...
		switch( $field['type'] ) {
			...
			case 'number' :
				?><input type="text" name="<?php echo $this->so_get_field_name($name, $repeater) ?>" value="<?php echo esc_attr($value) ?>" class="widefat siteorigin-widget-input siteorigin-widget-input-number" /><?php
				break;
